...
userRouter.get("/user/getAllUsers", userAuth, async (req, resp) => {
  try {
    ...
    resp.status(400).send("Something went wrong while getting users Data");
  }
});
...
userRouter.patch("/user/updateUser", userAuth, async (req, resp) => {
  try {
    ...
    resp.json({
      message: `${actualuserData.firstName}'s data has updated successfully`,
    });
  } catch {
